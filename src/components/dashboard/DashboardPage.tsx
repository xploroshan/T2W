...
export function DashboardPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center pt-24">
        <div className="text-center">
          <LogIn className="mx-auto h-16 w-16 text-t2w-border" />
  ...
  const userBadges = user.earnedBadges.map((eb) => eb.badge);
  ...
  return (
    <div className="min-h-screen pt-24 pb-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="card mb-8">
            <div className="flex items-center justify-between">
              ...
              <div
                ...
                style={{ backgroundColor: `${nextBadge.color}15` }}
              >
                {(() => {
                  const Icon = badgeIcons[nextBadge.icon] || Shield;
                  return <Icon className="h-7 w-7" style={{ color: nextBadge.color }} />;
                })()}
              </div>
            </div>
            <div className="mt-4">
              ...
              <div className="mt-1.5 h-3 overflow-hidden rounded-full bg-t2w-surface-light">
                <div
                  ...
                  style={{
                    width: `${progress}%`,
                    background: `linear-gradient(90deg, ${prevBadge?.color || "#e94560"}, ${nextBadge.color})`,
                  }}
                />
              </div>
            </div>
        ...
        {/* Tab Content */}
        {loading && activeTab !== "badges" && activeTab !== "bikes" ? (
          ...
          <>
                ...
                <div className="card">
                  ...
                  <div className="space-y-3">
                    {upcomingRides.slice(0, 3).map((ride) => (
                      <Link key={ride.id} href={`/ride/${ride.id}`} className="flex items-center gap-4 rounded-xl bg-t2w-surface-light p-3 transition-colors hover:bg-t2w-border/50">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-white truncate">{ride.title}</p>
                          <p className="text-xs text-t2w-muted">
                            {new Date(ride.startDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })} · {ride.distanceKm} km
                          </p>
                        </div>
                    ...
                    {upcomingRides.length === 0 && (
                      <p className="text-sm text-t2w-muted py-4 text-center">No upcoming rides</p>
                    )}
                  </div>
                </div>
                ...
                <div className="card">
                  ...
                  <div className="space-y-3">
                    ...
                    {completedRides.length === 0 && (
                      <p className="text-sm text-t2w-muted py-4 text-center">No completed rides yet</p>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  {completedRides.map((ride) => (
                    <Link key={ride.id} href={`/ride/${ride.id}`} className="card-interactive flex items-center gap-6">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-display text-lg font-bold text-white">{ride.title}</h4>
                  ...
                  {completedRides.length === 0 && (
                    <p className="text-center text-t2w-muted py-8">No rides completed yet. Join an upcoming ride!</p>
                  )}
                </div>
              </div>
            )}
            ...
            {activeTab === "badges" && (
              <div>
                <h3 className="mb-6 font-display text-xl font-bold text-white">T2W Achievement System</h3>
                <p className="mb-8 text-t2w-muted">
                  Earn badges by covering kilometers on T2W rides. The more you ride, the higher you climb!
                </p>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {BADGE_TIERS.map((badge) => {
                    const earned = user.earnedBadges.find((b) => b.badge.tier === badge.tier);
                    const Icon = badgeIcons[badge.icon] || Shield;
                    ...
                    return (
                      <div
                        ...
                        className={`card relative overflow-hidden transition-all ${
                          earned ? "border-opacity-50" : "opacity-60 grayscale"
                        } ${isNext ? "ring-2 ring-offset-2 ring-offset-t2w-dark" : ""}`}
                        style={{
                          borderColor: earned ? badge.color : undefined,
                          ...(isNext ? ({ "--tw-ring-color": badge.color } as React.CSSProperties) : {}),
                        }}
                      >
                        {isNext && (
                          <div className="absolute inset-x-0 top-0 h-1" style={{ backgroundColor: badge.color }} />
                        )}
                        <div className="flex items-start gap-4">
                          <div
                          ...
                          <div>
                            <h4 className="font-display text-lg font-bold text-white">{badge.name}</h4>
                            <p className="mt-1 text-xs text-t2w-muted">{badge.minKm.toLocaleString()} km required</p>
                            {earned && (
                              <p className="mt-1 text-xs font-medium" style={{ color: badge.color }}>
                                Earned {new Date(earned.earnedDate).toLocaleDateString("en-IN", { month: "short", year: "numeric" })}
                              </p>
                            )}
                            {isNext && (
                              <p className="mt-1 text-xs font-medium text-t2w-gold">
                                {(badge.minKm - user.totalKm).toLocaleString()} km to go
                              </p>
                            )}
                          </div>
                        </div>
                        <p className="mt-3 text-sm text-t2w-muted">{badge.description}</p>
                      </div>
                    );
                  })}
                </div>
